...
#define immJ()                                                                                                             \
  do                                                                                                                       \
  {                                                                                                                        \
    *imm = SEXT((BITS(i, 19, 12) << 12) + (BITS(i, 20, 20) << 11) + (BITS(i, 30, 21) << 1) + (BITS(i, 31, 31) << 20), 21); \
  } while (0)
#define immB()                                                                                                         \
  do                                                                                                                   \
  {                                                                                                                    \
    *imm = SEXT((BITS(i, 30, 25) << 5) + (BITS(i, 31, 31) << 12) + (BITS(i, 11, 8) << 1) + (BITS(i, 7, 7) << 11), 13); \
  } while (0)
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)         \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq, B, s->dnpc = src1 != src2 ? s->snpc : s->pc + imm);
  ...
  INSTPAT("0000000 ????? ????? 111 ????? 01100 11", and, R, R(rd) = src2 & src1);
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div, R, {
    ...
    else if ((sword_t)src1 == INT32_MIN && (sword_t)src2 == -1)
    {

      R(rd) = src1; // 返回被除数 (INT_MIN)
    }
    else
    {
  ...
  INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul, R, R(rd) = src2 * src1);
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh, R, int64_t mul = SEXT(src2, 32) * SEXT(src1, 32); R(rd) = BITS(mul, 63, 32));
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu, R, uint64_t mul = (uint64_t)src2 * (uint64_t)src1; R(rd) = BITS(mul, 63, 32));
  ...
  INSTPAT("??????? ????? ????? 010 ????? 00100 11", slti, I, R(rd) = (sword_t)imm > (sword_t)src1 ? 1 : 0);
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu, I, R(rd) = BITS(imm, 11, 0) > src1 ? 1 : 0);
  INSTPAT("0000000 ????? ????? 011 ????? 01100 11", sltu, R, R(rd) = src2 > src1 ? 1 : 0);
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal, J, R(rd) = s->pc + 4, s->dnpc = s->pc + imm); //+4 or not
  INSTPAT("??????? ????? ????? ??? ????? 11001 11", jalr, I, R(rd) = s->pc + 4, s->dnpc = (src1 + imm) & ~1);
  INSTPAT("0000000 ????? ????? 100 ????? 01100 11", xor, R, R(rd) = src2 ^ src1);
  INSTPAT("??????? ????? ????? 100 ????? 00100 11", xori, I, R(rd) = imm ^ src1);
  INSTPAT("0000000 ????? ????? 110 ????? 01100 11", or, R, R(rd) = src2 | src1);
  INSTPAT("??????? ????? ????? 110 ????? 00100 11", ori, I, R(rd) = imm | src1);
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrs, I, R(rd) = CSR(imm); CSR(imm) = src1;);
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall, I, assert(0););
